...
export const Plane = () => {
  ...
  const fragmentShader = `
    ...
    float line(float width, vec3 step){
        highp float testy = 20.;
        vec3 clone = pos;
        clone.y = (pos.y + u_time * testy);
        vec3 coord = clone / step;
    ...
    void main() {
      float v = line(1., vec3(5.));      
      vec3 c = vec3(0., 0., .5);
      c = mix(vec3(0., 0.0, 0.), c, v);
  ...
  const vertexShader = `
        ...
        void main() {
              ...
              pos = position;
             vec3 p = position;
            float displacement = u_amplitude * cnoise( u_frequency * position + u_time * 0.3);
  ...
  return (
    <mesh
      ...
      scale={[1, 1, 1]}
    >
      <planeGeometry attach="geometry" args={[2000, 1000, 300, 300]} />
      <shaderMaterial
        uniforms={uniforms}
        attach="material"
